var text: any = <HTMLElement><any> (document.getElementsByTagName("p"));
text = text[0];
var userLives = 0;
var botLives = 0;

function game(choice: any){
  let result: string = '';
  var computerChoice = Math.random();
  var userChoice = choice.id;
  let computerChoiceName: string;
  computerChoice = Math.floor((computerChoice * 5) + 1);
  if(computerChoice == 1){computerChoiceName = "Piedra";}
  else if(computerChoice == 2){computerChoiceName = "Papel";}
  else if(computerChoice == 3){computerChoiceName = "Tijera";}
  else if(computerChoice == 4){computerChoiceName = "Lagarto";}
  else{computerChoiceName = "Spock";} 
  ...
  if(result==userWins){
    (<HTMLElement><any>text).style.color="green";
    choice.style.background="green";
    (<HTMLElement><any>text).innerHTML=result;
    setTimeout(function(){choice.style.background ="";},1000);
  }
  else if(result==botWins){
    (<HTMLElement><any>text).style.color="red";
    choice.style.background="red";
    text.innerHTML=result;
    setTimeout(function(){choice.style.background ="";},1000);
  }
  else if(result==draw){
    (<HTMLElement><any>text).style.color="white";
...
function score(result:string, userWins:string, botWins:string){
  ...
  if(result==userWins){
    (<HTMLElement><any>life[(botLives++)+3]).style.color="black";
  }
  else if(result==botWins){
    (<HTMLElement><any>life[(userLives++)]).style.color="black";    
  }
  if(botLives==3 || userLives==3){
    ...
    for (var i = life.length - 1; i >= 0; i--) {
      (<HTMLElement><any>life[i]).style.color="rgb(255,0,0)";
      botLives=0
      userLives=0
    }
  }
